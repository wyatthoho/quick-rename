...
class App:
    ...
    def create_button(self, master: tk.Frame, row: int, col: int, text: str, command: Callable) -> tk.Button:
        button = tk.Button(
    ...
    def create_checkbutton(self, master: tk.Frame, row: int, col: int, text: str, command: Callable, variable: tk.IntVar) -> tk.Checkbutton:
    ...
    def create_radiobutton(self, master: tk.Frame, row: int, col: int, text: str, variable: tk.IntVar, value: int) -> tk.Radiobutton:
    ...
    def create_label_frame_target_directory(self):
        labelframe = self.create_label_frame(
            self.root, 0, 0, 'Choose the directory'
        )
        ...
        frame_down = self.create_frame(labelframe, 1, 0)
        intvar_applyto = tk.IntVar(value=1)
        self.create_label(frame_down, 0, 0, 'Apply to:')
        self.create_radiobutton(frame_down, 0, 1, 'Files', intvar_applyto, 1)
        self.create_radiobutton(frame_down, 0, 2, 'Folders', intvar_applyto, 2)

        frame_right = self.create_frame(labelframe, 0, 1, 2)
        self.create_button(
            frame_right, 0, 0, 'Choose',
            lambda: logic.choose_target_directory(self.logic_widgets)
        )
        self.create_button(
            frame_right, 1, 0, 'Read',
    ...
    def create_label_frame_renaming_method(self):
        ...
        frame_down = self.create_frame(labelframe, 2, 0)
        intvar_make_order = tk.IntVar()
        self.create_checkbutton(
            frame_down, 1, 0, 'Make an order',
            lambda: logic.config_order(self.logic_widgets), intvar_make_order
        )
        self.create_label(frame_down, 1, 1, 'Sep:')
        intvar_sep = tk.IntVar(value=1)
        radiobutton_prefix_1 = self.create_radiobutton(
            frame_down, 1, 2, '_', intvar_sep, 1
        )
        radiobutton_prefix_2 = self.create_radiobutton(
            frame_down, 1, 3, '-', intvar_sep, 2
        )
        radiobutton_prefix_3 = self.create_radiobutton(
